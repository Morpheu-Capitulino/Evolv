...
import { 
  ...
  Calendar, History, X, RotateCcw, AlertTriangle, CheckCircle2, Zap, Trophy
} from 'lucide-react';
...
export default function ProgressoPage() {
  ...
  const scoreBase = Math.round((musculosEmEvolucao / totalMusculos) * 100);
    ...
    <div className="progresso-page fade-in">
      ...
      <div className="treino-content main-scroll">
        ...
        <div className="workout-tabs" style={{marginBottom: '15px'}}>
          <button className={`tab-btn ${activeTab === 'mapa' ? 'active' : ''}`} onClick={() => setActiveTab('mapa')}>Análise Inteligente</button>
          <button className={`tab-btn ${activeTab === 'medidas' ? 'active' : ''}`} onClick={() => setActiveTab('medidas')}>Dados de Evolução</button>
        </div>

        {loading ? ( 
          <div className="center-all" style={{marginTop: '50px', flexDirection: 'column'}}>
            <Activity className="spin" color="var(--evolv-green)" size={40}/>
            <p style={{color: 'var(--evolv-green)', marginTop: '15px', fontWeight: 'bold'}}>🔄 Analisando o seu progresso...</p>
          </div> 
        ) : (
          <>
            {activeTab === 'mapa' && (
              <div className="fade-in">
                ...
                {resumoIA && (
                  <div className="glass-card" style={{padding: '15px', marginBottom: '15px', borderLeft: '4px solid var(--evolv-green)', background: 'rgba(58, 181, 74, 0.05)'}}>
                    <p style={{margin: 0, color: '#fff', fontSize: '0.95rem', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '8px'}}>
                      <Zap size={18} color="var(--evolv-green)" /> {resumoIA}
                    </p>
                  </div>
                )}

                {comparativo && parseFloat(comparativo.waist.diff) > 0 && (
                  <div className="glass-card danger fade-in" style={{padding: '12px', marginBottom: '15px', borderLeft: '4px solid #ff4d4d', display: 'flex', alignItems: 'center', gap: '10px'}}>
                    <AlertTriangle color="#ff4d4d" size={20} flexShrink={0} />
                    <p style={{margin: 0, fontSize: '0.85rem', color: '#fff'}}>Aumento de gordura abdominal detetado. Considere rever o volume calórico.</p>
                  </div>
                )}
                ...
                {evolucaoTotal && (
                  <div style={{display: 'flex', gap: '10px', marginBottom: '15px'}}>
                    <div className="glass-card" style={{flex: 1, padding: '12px', textAlign: 'center'}}>
                      <span style={{fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '4px'}}>PESO TOTAL (DESDE INÍCIO)</span>
                      <strong style={{color: evolucaoTotal.peso <= 0 ? 'var(--evolv-green)' : '#ff4d4d', fontSize: '1.1rem'}}>{evolucaoTotal.peso > 0 ? '+' : ''}{evolucaoTotal.peso} kg</strong>
                    </div>
                    <div className="glass-card" style={{flex: 1, padding: '12px', textAlign: 'center'}}>
                      <span style={{fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '4px'}}>GORDURA (DESDE INÍCIO)</span>
                      <strong style={{color: evolucaoTotal.bf <= 0 ? 'var(--evolv-green)' : '#ff4d4d', fontSize: '1.1rem'}}>{evolucaoTotal.bf > 0 ? '+' : ''}{evolucaoTotal.bf} %</strong>
                    </div>
                  </div>
                )}
                ...
                {melhorMetrica && (
                  <div className="glass-card" style={{padding: '15px', marginTop: '15px', display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255, 170, 0, 0.05)', border: '1px solid rgba(255, 170, 0, 0.3)'}}>
                    <Trophy size={24} color="#ffaa00" />
                    <div>
                      <span style={{fontSize: '0.75rem', color: '#ffaa00', textTransform: 'uppercase', fontWeight: 'bold'}}>DESTAQUE DA AVALIAÇÃO</span>
                      <strong style={{display: 'block', color: '#fff', fontSize: '1.05rem'}}>{melhorMetrica.nome} melhorou {melhorMetrica.valor} {melhorMetrica.nome === 'Gordura' ? '%' : 'cm'}</strong>
                    </div>
                  </div>
                )}
                ...
                <div className="glass-card" style={{padding: '20px', marginTop: '15px', display: 'flex', alignItems: 'center', gap: '20px'}}>
                  <div className="score-circle" style={{position: 'relative', width: '80px', height: '80px'}}>
                    <svg viewBox="0 0 36 36" style={{width: '100%', height: '100%'}}>
                      ...
                      <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="var(--evolv-green)" strokeWidth="3" strokeDasharray={`${simetriaScore}, 100`} style={{transition: 'stroke-dasharray 1.5s ease-out'}} />
                    </svg>
                  ...
                  {Object.keys(musculosStatus).map(key => {
                    ...
                    if (musculosStatus[key] === 1 && modelView === 'posterior') return (
                      <div key={key} className="glass-card" style={{borderLeft: '4px solid #ff4d4d', padding: '15px', display: 'flex', gap: '15px', alignItems: 'center'}}>
                        ...
                        <div><strong style={{color: '#fff', display: 'block'}}>{nomesMusculos[key]}</strong><span style={{fontSize: '0.85rem', color: 'var(--text-muted)'}}>Estagnação ou perda detetada. A IA irá aumentar a carga no próximo treino.</span></div>
                      </div>
                    );
                    return null;
                  })}
                ...
                <div className="metrics-grid-pro">
                  {comparativo ? (
                    <>
                     ...
                     <div className="empty-state-pro" style={{gridColumn: '1 / -1'}}><p style={{color: 'var(--text-muted)'}}>Ainda não tens medidas registadas. Adicione uma avaliação para ver a evolução!</p></div>
                  )}
                </div>

                {data?.getMyMeasurements?.length > 0 && (
                  <div className="glass-card" style={{marginTop: '20px', padding: '15px'}}>
                    <h4 style={{fontSize: '0.9rem', color: '#fff', marginBottom: '10px'}}>Registo Recente</h4>
                    <div style={{display: 'flex', flexDirection: 'column', gap: '8px'}}>
                      {data.getMyMeasurements.slice(0,5).map(m => (
                        <div key={m.id} style={{display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-muted)', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)'}}>
                          <span>{new Date(m.date).toLocaleDateString('pt-BR')}</span>
                          <strong style={{color: '#fff'}}>{m.weight} kg</strong>
                        </div>
                      ))}
                    </div>
                  </div>
